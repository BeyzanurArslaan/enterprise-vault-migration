"""End-to-end regression tests for checkpointing with parallel upload.

This module exercises the concrete migration pipeline with a bounded worker
count to verify that the uploaded results, checkpoint snapshots, and final
metrics remain deterministic when the upload step uses concurrent workers.
"""
...
def _build_retention_policy() -> RetentionPolicy:
    """Create a deterministic retention policy for the parallel upload test."""
...
def _build_mail_items() -> list[MailItem]:
    """Create two deterministic mail items for the parallel upload test."""

    timestamp = datetime(2026, 1, 1, 12, 0, tzinfo=UTC)
    return [
        MailItem(
            subject="Quarterly Report A",
        ...
        MailItem(
            subject="Quarterly Report B",
            sender="sender@example.com",
            body="Body B",
            received_at=timestamp,
            sent_at=timestamp,
            modified_at=timestamp,
            internet_message_id="message-2",
...
def _build_vault_stores() -> tuple[VaultStore, ...]:
    """Create a deterministic source dataset with two mail items."""

    mailbox = Mailbox(address="alice@example.com", mail_items=_build_mail_items())
...
def _build_initial_context(started_at: datetime) -> MigrationStepContext:
    """Create the initial migration context with parallel upload enabled."""
...
def test_parallel_upload_migration_persists_a_deterministic_final_checkpoint() -> None:
    """The concrete pipeline should remain deterministic under parallel upload."""
    ...
    assert final_checkpoint.processed_items == 2
    assert final_checkpoint.successful_items == 2
    assert final_checkpoint.uploaded_items == 2
    assert final_checkpoint.upload_worker_count == 2
